...
# The below organises the training data in an alternating fashion; w_1, u_1, w_2, u_2 etc
# Consider re-structuring, although will need to retrain the emulator
# UPDATE AS IN EMULATOR CODE!!!
...
K_eta = as.matrix(read.table("outputs/basis_50x3_multi.csv", sep = ',', header = TRUE))
...
exp_displacement = cbind(experimental_data$W,experimental_data$U)
...
for (i in 1:n_y){
  ...
  mu_y[i,1] = sum(bases[1,]*bases[2,]*mu_dt[as.numeric(connectivity[y_element[i]+1,])+2])/4.0
  mu_y[i,2] = sum(bases[1,]*bases[2,]*mu_dt[as.numeric(connectivity[y_element[i]+1,])+(n_eta/2)+2])/4.0
}
...
for (i in 1:n_y) {
  ...
  K_y[n_y + i,] = colSums(matrix(bases[1,]*bases[2,],4,p_eta)*K_eta[as.numeric(connectivity[y_element[i]+1,])+(n_eta/2)+2,])/4.0
}

output_frame = cbind(experimental_data[c("X","Y","Z")],K_y[1:n_y,],K_y[(n_y+1):(2*n_y),])
for (i in 1:p_eta) {
  colnames(output_frame)[3+i] = sprintf("K_y_w,%d",i)
  colnames(output_frame)[3+p_eta+i] = sprintf("K_y_u,%d",i)
}
...
# This is the point at which the options diverge. It would be good to have 
# multiple options with conditional statement, i.e. same error all components,
# different, etc.
...
a_y_dash = c(5.0, 5.0)
b_y_dash = c(0.05, 0.05)

# Array containing separated inner product for different displacement components 
# Arranged as required for input to stan
# automate later
B_w = B[(1:n_y),]
B_u = B[-(1:n_y),]
BTWyB_w = t(B_w)%*%W_y[(1:n_y),(1:n_y)]%*%B_w
BTWyB_u = t(B_u)%*%W_y[-(1:n_y),-(1:n_y)]%*%B_u
# BTWyB_w_inv = solve(BTWyB_w)
# BTWyB_u_inv = solve(BTWyB_u)

# FROM HERE >>> THE CODE BELOW IS THE INCORRECT FORMAT FOR INPUT TO STAN. SEE:
# https://discourse.mc-stan.org/t/dimension-mismatch-when-passing-array-of-matrix-from-r-to-stan/22404/8
# BECAUSE I'M TREATING THE ERROR IN MY TWO COMPONENTS AS INDEPENDENT, I CAN SEPARATE
# OUT THE ALGEBRA FROM THE SAMPLING (NOT POSSIBLE IN GENERAL). # TO DO THIS I 
# NEED TO SEPARATE OUT THE DIFFEREINT k_y,W^t * k_y,w etc terms. I'm passing these
# in as an array of matrices. Not sure if this will work. Next step is to modify
# the calculation of Sigma_z_hat in line with my algebra

# Try not to be too precious with the coding, this might not even work!!!
# It's also possible that for higher loads having a single errror parameter is 
# sufficient (probs better as accounts for correlation also....)
BTWyB_w_check = BTWyB_w
BTWyB_u_check = BTWyB_u
BTWyB_w = array(BTWyB_w, dim = c(1, p_eta, p_eta))
BTWyB_u = array(BTWyB_u, dim = c(1, p_eta, p_eta))

BTWyB_sep <- abind(BTWyB_w,BTWyB_u,along=1)
...
p_eta_em = 5
q_y = 2 # number of output dimensions (when sorted out, move higher up the order)

# emulator_parameters = c(as.matrix(read.table("outputs/emulator_modes_unit.csv", sep = ',', header = TRUE)))
emulator_parameters = c(as.matrix(read.table("outputs/emulator_modes_50x3_multi.csv", sep = ',', header = TRUE)))
...
stan_data = list(m=m, q=q, n_eta=n_eta, n_y=n_y, q_y=q_y, p_eta=p_eta, a_y_dash = a_y_dash, b_y_dash = b_y_dash, lambda_eta = lambda_eta, z_hat = z_hat, tf_param_1=tf_param_1, tf_param_2=tf_param_2, rho_w = rho_w, lambda_w = lambda_w, tc = tc, KTKinv = KTKinv, BTWyB = BTWyB_sep)
fit = stan(file = "source/Full_Field_Calibration_Higdon_Prior_Spec_No_Disc_fixed_param_multi.stan",
           data = stan_data,
           iter = 4000,
           chains = 3)
#           iter = 1,
...
# Plot precision of the observation error
lambda_plot = seq(0,5.0, length.out = 1000)
dev.new(noRStudioGD = TRUE)
par(mfrow = c(1,2))
for (i in 1:2){
  #plot posterior
  hist(samples$lambda_y[,i],
       main = "lambda_y",
       xlab = "lambda_y",
       col = "firebrick1",
       breaks = 25,
...
eta_y = matrix(0, 2*n_y, N_sam_plot)
...
for (i in 1:N_sam_plot){
  ...
  eta_y[,i] = (K_y %*% w_star[,i])*sd_dt + as.vector(mu_y)
}
