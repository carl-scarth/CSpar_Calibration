...
K_eta = as.matrix(fread(paste("outputs/basis_nonlinear_",in_file,"_uvw180z.csv", sep = "")))
...
emulator_parameters = c(as.matrix(read.table(paste("outputs/nonlinear_emulator_modes_",in_file,"_uvw180z_mod.csv", sep=""), sep = ",", header = TRUE)))
if (use_subset) {
...
calibration_inp_hist(tf_trans, tf_param = tf_param, nrows=2)
