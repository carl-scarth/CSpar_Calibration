...
exp_data_file = "Interpolated_DIC_187kN_no0" # Identifier of file containing DIC data
surface_elements = "new_spar_mesh_outer_surface_elements" # File identifier string for surface mesh connectivity
p_sub = 4 # Use a subset of the basis functions
use_subset = TRUE
...
abaqus_displacements <- fromJSON(file = paste("inputs/",in_file,"_correct_output_struct_187kN.json", sep=""))
...
K_eta = as.matrix(fread(paste("outputs/basis_nonlinear_",in_file,"_uw_corr.csv", sep = "")))
...
emulator_parameters = c(as.matrix(read.table(paste("outputs/nonlinear_emulator_modes_",in_file,"_uW_corr.csv", sep=""), sep = ",", header = TRUE)))
if (use_subset) {
