...
K_eta = as.matrix(fread(paste("outputs/basis_nonlinear_",in_file,"_w150z.csv", sep = "")))
...
processed_data = reduce_dimension_calibration_multi_fov(y, K_y, c(n_y1, n_y2))
...
emulator_parameters = c(as.matrix(read.table(paste("outputs/nonlinear_emulator_modes_",in_file,"_w150z_mod.csv", sep=""), sep = ",", header = TRUE)))
if (use_subset) {
