...
output_file = "gp_predictions_nonlinear_" + infile + "_u.json"
                ...
                for k, post_sam in enumerate(predictions["Posterior_Sample"]):
                    ...
                    if n_pred > 1:
                        if QoI+"_sam_"+str(k) not in output_RP:
                            ...
                            output_max[QoI+"_sam_"+str(k)] = np.empty((n_pred, n_frames))

                        output_RP[QoI+"_sam_"+str(k)][i, j] = post_sam[1]
                        output_max[QoI+"_sam_"+str(k)][i, j] = post_sam[max_ind]
                    else:
                        if QoI not in output_RP:
                            output_RP[QoI] = np.empty((n_frames, n_post_sam))
                            output_max[QoI] = np.empty((n_frames, n_post_sam))
                        output_RP[QoI][j,k] = post_sam[1]
                        output_max[QoI][j,k] = post_sam[max_ind]
...
if n_pred > 1:
    ...
    if len([key for key in output_RP.keys() if "eta_mu_sam" in key]) > 0:
        ...
        ax.set_title("Posterior sample displacement at reference point")
        ...
        for key, value in output_max.items():
            ...
            if "eta_sam_sam" in key:
                for displacement in value:
                    ax2.plot(-displacement, np.linspace(0,force,n_frames), "c", linewidth=0.25)
            if "eta_sigma_sam" in key:
                mu_key = "eta_mu_sam_" + key.strip("eta_sigma_sam_")
        ...
        ax2.set_title("Posterior sample displacement at node " + str(max_ind))
