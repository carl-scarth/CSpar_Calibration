...
in_file = "LHSDesign100x10_1" # File identifier string for input and output csvs
exp_data_file = "Interpolated_DIC_180_zeroed" # Identifier of file containing DIC data
...
abaqus_displacements <- fromJSON(file = paste("inputs/",in_file,"_output_struct_180kN_zeroed.json", sep=""))
...
K_eta = as.matrix(fread(paste("outputs/basis_nonlinear_",in_file,"_u180z.csv", sep = "")))
...
# W_y = rep(1.0,n_y)
W_y = rep(1/(sigma_noise^2),n_y*q_y)
...
emulator_parameters = c(as.matrix(read.table(paste("outputs/nonlinear_emulator_modes_",in_file,"_u180z_mod.csv", sep=""), sep = ",", header = TRUE)))
if (use_subset) {
...
calibration_inp_hist(tf_trans, tf_param = tf_param, inp_labels = labels, nrows=2)
