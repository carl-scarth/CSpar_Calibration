...
disp_str = c("w") # String which identifies the displacement component of interest (u,v, or w)
q_y = length(disp_str)
...
exp_data_file = "Interpolated_DIC_multistep_160kN_LC" # Identifier of file containing DIC data
...
K_eta = as.matrix(fread(paste("outputs/basis_nonlinear_",in_file,"_w160z.csv", sep = "")))
...
emulator_parameters = c(as.matrix(read.table(paste("outputs/nonlinear_emulator_modes_",in_file,"_w160z_mod.csv", sep=""), sep = ",", header = TRUE)))
if (use_subset) {
