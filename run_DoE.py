...
sym = True
# layup = [45.0, -45.0, 90.0, 0.0, 45.0, -45.0, 90.0, 0.0, 45.0, -45.0, 90.0, 0.0] # For shell mesh (doubling up on plies to reduce time as no failure in this model. Also there is symmetry)
...
for i, x_i in iterable:
    ...
    if shell_mesh:
        write_shell_parameters(t_ply=t_ply, Zlength=Zlength, height=height, LFlange_theta=LFlange_theta, RFlange_theta=RFlange_theta, n_plies=n_plies, rotate_flanges = rotate_flanges, model_name=file_str)
    else:
    ...
    # comprised of shells
    if shell_mesh:
        write_shell_inp(E11=E11, E22=E22, nu12=nu12, nu23=nu23, G12=G12, t_ply=t_ply, K=K, x_spring=x_spring, load=load, StackSeq = layup, init_inc=init_inc, min_inc=min_inc, max_inc=max_inc)
    else:
