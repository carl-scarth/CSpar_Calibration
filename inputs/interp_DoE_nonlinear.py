...
if __name__ == "__main__":
    ...
    for i,sample in enumerate(sample_dict["Sample"]):
        ...
        if output_mean:
            print(pd.DataFrame(newcol, columns = ["w_" + str(i)]))
            
            interp_outputs = pd.concat([interp_outputs, pd.DataFrame(newcol, columns = ["disp_" + str(i)]), pd.DataFrame(force, columns = ["force_"+str(i)])], axis=1)
