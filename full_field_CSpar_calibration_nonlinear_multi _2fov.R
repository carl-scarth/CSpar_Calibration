...
K_eta = as.matrix(fread(paste("outputs/basis_nonlinear_",in_file,"_uw160z.csv", sep = "")))
...
emulator_parameters = c(as.matrix(read.table(paste("outputs/nonlinear_emulator_means_",in_file,"_uw160z_mod.csv", sep=""), sep = ",", header = TRUE)))
if (use_subset) {
...
lambda_y_t = lambda_y
lambda_y_t[,1] = lambda_y[,1]/(sd_dt[1]^2)
lambda_y_t[,3] = lambda_y[,3]/(sd_dt[1]^2)
lambda_y_t[,2] = lambda_y[,2]/(sd_dt[n_eta]^2)
lambda_y_t[,4] = lambda_y[,4]/(sd_dt[n_eta]^2)
lambda_hist(lambda_y_t, prior_shape = a_y, prior_rate = b_y, label = "lambda_y") #, adj_prior_shape = a_y_dash, adj_prior_rate = b_y_dash)

par(mfrow = c(1,4))
# Get maximum and minimum x-limit for plots
max_x_prior <- qgamma(0.9999, a_y, b_y)
min_x_prior <- qgamma(0.01, a_y, b_y)
scaler = c(sd_dt[1]^2, sd_dt[n_eta]^2, sd_dt[1]^2, sd_dt[n_eta]^2)
scaler = c(1,1,1,1)
max_x_prior = c(max_x_prior/scaler)

#min_x_prior = c(min_x_prior/sd_dt[1]^2, min_x_prior/sd_dt[n_eta]^2,min_x_prior/sd_dt[1]^2, min_x_prior/sd_dt[n_eta]^2)
min_x_prior = c(0,0,0,0)
#ylim = c(0.012, 1e-4, 0.01, 1e-4)
ylim = c(0.014,0.01,0.012,0.01)
#xlim = c(400, 40000, 400, 40000)
xlim = c(350, 350, 350, 350)
# Could just be zero
for (i in 1:4) {
  header = paste("lambda_y", i, sep="_")
  #plot posterior
  #hist(lambda_y_t[,i],
  hist(lambda_y[,i],
       main = header,
       ...
       cex.axis=1.5,
       cex.lab=1,5)
  lambda_plot <- seq(min_x_prior[i], max_x_prior[i], length.out = 1000) # set of x values for prior plot
  prior_plot = scaler[i]*dgamma(scaler[i]*lambda_plot, shape=a_y, rate=b_y)
  lines(lambda_plot,prior_plot,lwd=3,col="blue")
}


sd_epsilon = 1/sqrt(lambda_y)
sd_epsilon[,1] = sd_epsilon[,1]*sd_dt[1]
sd_epsilon[,3] = sd_epsilon[,3]*sd_dt[1]
sd_epsilon[,2] = sd_epsilon[,2]*sd_dt[n_eta]
sd_epsilon[,4] = sd_epsilon[,4]*sd_dt[n_eta]

par(mfrow = c(1,ncol(sd_epsilon)))
for (i in 1:ncol(sd_epsilon)){
  hist(sd_epsilon[,i])
}
write.csv(sd_epsilon, paste("E:/Calibration_outputs_for_paper/",in_file,"_observation_error_samples.csv",sep = ""), row.names = FALSE)
...
calibration_inp_hist(tf_trans, tf_param = tf_param, nrows=2, new_window = T)
...
prior_posterior_pairs(tf_trans, tf_param, new_window = T)
# Output samples
tf_trans = data.frame(tf_trans)
colnames(tf_trans) = labels
write.csv(tf_trans, paste("E:/Calibration_outputs_for_paper/",in_file,"_posterior_samples.csv",sep = ""), row.names = FALSE)    
...
write(out_json, paste("E:/Calibration_outputs_for_paper/gp_predictions_nonlinear_",in_file,"obs_err0.5.json",sep=""))
