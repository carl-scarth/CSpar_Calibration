...
data {
  ...
  int<lower=0> q_y;             // number of output components
  ...
  vector<lower=0>[q_y] a_y_dash; // Adjusted value of the shape parameter for the lambda_y prior in accordance with the normal-gamma model of Higdon et al. Here there is a different error for each component
  vector<lower=0>[q_y] b_y_dash; // Adjusted value of the rate parameter for the lambda_y prior in accordance with the normal-gamma model of Higdon et al. Here there is a different error for each component
  ...
  matrix[p_eta,p_eta] BTWyB[q_y]; // Inverse of product B'*W_y*B, where B is a matrix containing emualtor basis terms, as outlined in Higdon et al., and W_y is the observation error precision matrix
}
...
parameters {
  ...
  // characterised prior
  row_vector<lower=-0.05,upper=1.05>[q-1] tf_gauss; // Gaussian-distributed priors
  row_vector<lower=0>[q_y] lambda_y;
...
model {
  ...
  matrix[p_eta, p_eta] Prec_y;      // Precision associated with the observation error expressed in the reduced dimensional space
  ...
  // Specify priors here
  for (i in 1:q_y){
    lambda_y[i] ~ gamma(a_y_dash[i], b_y_dash[i]);               // Precision parameter for observation error, gamma (shape, rate)
  }
