...
log_unif_pdf <- function(x, a, b){
  f = 1/(x*log(b/a))
  return(f)
}

# produce plot in log space
...
calibration_inp_hist <- function(tf, tf_param = NULL, inp_labels = NULL, new_window = FALSE, nrows=1){
  ...
  for (i in 1:q){
    ...
    ylim = matrix(c(0,0,0,0,0,0,0,0,0.05, 0.8, 1.5, 100, 0.9, 2.0, 1.2, 3.5), nrow = 8, ncol=2)
    xaxp = matrix(c(110, 6, 3.5, 0.1, -3, 6, 4, 0, 
                    190, 12, 6.5, 0.15, 3, 12, 12, 1.6,
                    8, 6, 6, 8, 8, 6, 8, 4), nrow = 8, ncol = 3)
    yaxp = matrix(c(0, 0, 0, 0, 0, 0, 0, 0, 
             0.05, 0.8, 1.5, 100, 0.8, 2, 1.2, 3.5, 
             4, 4, 3, 4, 4, 4, 6, 7), nrow = 8, ncol = 3)
    hist(tf[,i], 
         main = label,
         xlab = label,
         col = "brown1",
      ...
      if (!is.null(tf_param)){
        ...
        if (tf_param$distribution[i] == "Gaussian" | tf_param$distribution[i] == "Lognormal" ){
          ...
          #prior_plot = log_unif_pdf(t_plot, tf_param$param_1[i], tf_param$param_2[i])
        }
        if (i == 1 | i == 2 | i == 3){
          lines(t_plot,prior_plot,lwd=3,"col"="blue", ylim = c(0, max(prior_plot)))
        } else {
...
lambda_hist <- function(lambda, prior_shape = 5.0, prior_rate = 5.0, label = "lambda", adj_prior_shape = NULL, adj_prior_rate = NULL, new_window = FALSE){
  ...
  prior_posterior_pairs <- function(tf, tf_param, inp_labels = NULL, new_window = FALSE){
    ...
    pairs(prior_post_frame,col=colours, cex.labels=2, cex.axis=1.75, gap=0.05, pch = 20)
  }
