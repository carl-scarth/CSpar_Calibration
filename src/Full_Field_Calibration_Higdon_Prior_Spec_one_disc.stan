...
data {
  ...
  vector[p_delta+(m+1)*p_eta] z_hat; // Vector of regression weights, representing both model output and discrepancy as defined in Higdon et al.
  vector[q] tf_param_1;              // prior means of calibration parameters
  vector[q] tf_param_2;           // prior standard deviations of calibration parameterss
  ...
  matrix[p_eta+p_delta,p_eta+p_delta] BTWyBinv; // Inverse of product B'*W_y*B, where B is a matrix containing emualtor and discrepancy basis terms, as outlined in Higdon et al., and W_y is the observation error precision matrix
}

transformed data {
  vector[p_delta+(m+1)*p_eta] mu_z_hat;                // prior mean vector

  // Assemble joint vector of experimental and model outputs
  mu_z_hat = rep_vector(0, p_delta+(m+1)*p_eta);  // set prior mean vector to zero
}
...
model {
  ...
  matrix[p_delta+(m+1)*p_eta, p_delta+(m+1)*p_eta] sigma_z; // Covariance matrix for joint experimental and modelling data
  matrix[p_delta+(m+1)*p_eta, p_delta+(m+1)*p_eta] sigma_z_hat; // Covariance matrix for joint experimental and model data, adjusted with model and emulator error terms transformed into low-dimensional space
  matrix[p_delta+(m+1)*p_eta, p_delta+(m+1)*p_eta] L_z_hat;             // Cholesky decomposition of covariance matrix
    
  // Define the covariance for the discrepancy weights. Here I assume that all weights can be modelled
  // using the same set of hyperparameters, i.e. F = 1, |G_i| = p_delta following the notation of 
  ...
  sigma_z = rep_matrix(0,p_delta+(m+1)*p_eta, p_delta+(m+1)*p_eta);
  sigma_z[1:p_delta,1:p_delta] = sigma_v;
  sigma_z[p_delta+1:p_delta+p_eta,p_delta+1:p_delta+p_eta] = sigma_u;
  sigma_z[p_delta+p_eta+1:,p_delta+p_eta+1:] = sigma_w;
  sigma_z[p_delta+1:p_delta+p_eta,p_delta+p_eta+1:] = sigma_uw;
  sigma_z[p_delta+p_eta+1:,p_delta+1:p_delta+p_eta] = sigma_uw';
    
  // Adjust the covariance matrix sigma_z to include transformed emulator and experimental error terms
  sigma_z_hat = rep_matrix(0,p_delta+(m+1)*p_eta, p_delta+(m+1)*p_eta);
  sigma_z_hat[1:(p_delta+p_eta),1:(p_delta+p_eta)] = BTWyBinv/lambda_y;
  sigma_z_hat[(p_delta+p_eta+1):,(p_delta+p_eta+1):] = KTKinv/lambda_eta;
  ...
  lambda_v ~ gamma(1.0, 0.000001); // Precision parameter for discrepancy, gamma (shape, rate) (Higdon)
  //lambda_v ~ gamma(10.0, 0.0001); // Precision parameter for discrepancy, gamma (shape, rate) (Chong)
  ...
  for (i in 1:(q-1)){
    tf[i] ~ normal(tf_param_1[i], tf_param_2[i]);
  }
